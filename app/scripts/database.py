...
def create_database():
    print(str(datetime.now()) + ' - create_database')
...
def read_users():
    print(str(datetime.now()) + ' - read_users')
    users = []
    connection = sqlite3.connect(database)
    cursor = connection.cursor()
...
def init_last_posts():
    print(str(datetime.now()) + ' - init_last_posts')
    ...
    for source in sources_sites:
        ...
        print(str(datetime.now()) + ' - init ' + source['source_name'])
        if not is_exist:
            cursor.execute(
