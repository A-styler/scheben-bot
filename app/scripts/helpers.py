...
def get_all_posts_by_sources(sources):
    ...
    print(str(datetime.now()) + ' - get_all_posts_by_sources')
    try:
        raw_posts = {}
        for source in sources:
            print(str(datetime.now()) + ' - get_all_posts_by_sources ' + source['source_name'])
            headers = {'User-Agent': 'My User Agent 1.0'}
...
def get_new_posts_info(new_links):
    print(str(datetime.now()) + ' - get_new_posts_info')
    try:
        new_posts_info = []
        print(str(datetime.now()) + ' - get_new_posts_info_try')

        for source_name in new_links:
            for post in new_links[source_name]:
                print(str(datetime.now()) + ' - get_new_posts_info_try_for')
                time.sleep(5)
